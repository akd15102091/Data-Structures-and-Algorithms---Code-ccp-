...
void bottomViewElements(BTPTR T, BTPTR ARR[], int level=0, int hd=0){
    if(T==NULL){
        return;
    }
    T->level = level;
    bottomViewElements(T->lchild, ARR, level+1, hd-1);
    ...
    bottomViewElements(T->rchild, ARR, level+1, hd+1);
}

void bottomView(BTPTR Q[]){
