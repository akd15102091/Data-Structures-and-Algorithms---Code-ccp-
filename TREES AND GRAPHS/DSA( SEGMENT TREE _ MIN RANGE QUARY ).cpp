...
int min_range_quary( int st[] , int qlow , int qhigh , int low , int high ,int pos )
{
	...
	{
		return 100000 ;  //very big number  	
	}	

	int mid = (low + high) / 2   ;
	return (min( (min_range_quary(st , qlow , qhigh , low , mid , 2*pos +1)) , (min_range_quary(st , qlow , qhigh , mid+1 , high , 2*pos +2)) )) ;
}
...
//	else cout << "NOt p"
	...
	int qlow , qhigh ;
	cin>> qlow >> qhigh ;
	int min_value = min_range_quary( st , qlow , qhigh , 0 , temp-1 , 0 )  ;
	
	cout<<"In given range , min_value is : "<< min_value <<endl ;
	return 0 ;
}
// 7    //  -2 3 -4 -3 0 2 1
// 9 //  -4 3 -7 0 4 6 5 -1 -2
